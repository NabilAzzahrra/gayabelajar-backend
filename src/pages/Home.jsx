...
function Home() {
  ...
  const getResult = async (data) => {
    await axios
      .get(`https://gayabelajar-backend.politekniklp3i-tasikmalaya.ac.id/hasils/${data.id}`)
      .then((response) => {
  ...
  const startTest = async () => {
    ...
    try {
      const responseUserExist = await axios.get(
        `https://gayabelajar-backend.politekniklp3i-tasikmalaya.ac.id/users/${user.id}`
      );
      if (responseUserExist.data) {
        ...
        try {
          ...
          try {
            ...
            setUser(data);
            const responseUser = await axios.post(`https://gayabelajar-backend.politekniklp3i-tasikmalaya.ac.id/users`, data);
            if (responseUser) {
              setTimeout(() => {
                setLoading(false);
              }, 1000);
